#!/usr/bin/env python3
"""
mazemaker_inception_bench.py — The Unified Mazemaker Production Benchmark.
...
Usage:
    python mazemaker_inception_bench.py                    # full 500q run
    python mazemaker_inception_bench.py --limit 100        # quick sample
    python mazemaker_inception_bench.py --dream            # also run dream ablation
"""
...
def _pg_reset_schema(schema: str) -> None:
    ...
    Lets every inception_bench run start from a clean slate without touching the
...
def main():
    ...
    if args.read_cache:
        ...
        schema = (args.schema or
                  f"inception_bench_{datetime.now(timezone.utc).strftime('%Y%m%dt%H%M%S')}")
    schema = schema.lower()
    ...
    # ── 1. Load data ──────────────────────────────────────────────────────
    print(f"\n{'='*72}")
    print(f"  inception_bench — Unified Mazemaker Production Benchmark")
    ...
    # ── 6. Save results ────────────────────────────────────────────────────
    out = {
        "benchmark": "mazemaker_inception_bench",
        "schema": schema,
    ...
    out_path = RESULTS_DIR / f"inception_bench{tag}_{ts}.json"
    ...
    # `DROP SCHEMA "inception_bench_<ts>" CASCADE` if disk pressure matters.
    return 0
