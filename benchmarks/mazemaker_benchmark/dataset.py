...
class EpisodicGenerator(BaseGenerator):
    ...
    COMPONENTS = [
        ...
        "mazemaker.py", "sync_bridge.py", "test_suite.py", "dashboard.py",
...
class TemporalGenerator(BaseGenerator):
    ...
    COMPONENTS = ["mazemaker.py", "embed_provider.py", "api.py", "auth.py", "router.py"]
...
class GraphGenerator(BaseGenerator):
    ...
    KNOWLEDGE_CLUSTERS = {
        ...
        "models": [
            ...
            "FastEmbed ONNX model lives in ~/.mazemaker/engine/models/",
        ],
        "benchmarks": [
...
def load_or_generate_dataset(config=None, seed: int = 42) -> Tuple[List[Dict], List[Dict]]:
    ...
    `~/.mazemaker_benchmark/data/dataset.json` with no way to
    ...
    cache_path = Path(
        ...
        or "~/.mazemaker_benchmark/data/dataset.json"
    ).expanduser()
