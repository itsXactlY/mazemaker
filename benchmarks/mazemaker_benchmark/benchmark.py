...
Note:
    ...
    `mazemaker_benchmark/` directory) at the front of sys.path. Local
...
def _assert_not_hotpath(p) -> None:
    ...
    that you really want to write into ~/.mazemaker/engine/memory.db).
    """
...
def _new_db(cfg=None) -> str:
    ...
    # ~/.mazemaker/engine/memory.db, so the check was dead code that
...
Examples (run via the runner script which sets sys.path correctly):
  python benchmarks/mazemaker_benchmark/runner.py
  python benchmarks/mazemaker_benchmark/runner.py --suite retrieval
  python benchmarks/mazemaker_benchmark/runner.py --suite retrieval --suite gpu
  python benchmarks/mazemaker_benchmark/runner.py --dry-run
  python benchmarks/mazemaker_benchmark/runner.py --output-dir /tmp/results
  python benchmarks/mazemaker_benchmark/runner.py --seed 0
  python benchmarks/mazemaker_benchmark/runner.py --list

# F41 fix (audit 2026-05-13): the previous epilog showed
# `python -m benchmarks.mazemaker_benchmark.runner` which is NOT
