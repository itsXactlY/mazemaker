...
# ~/.mazemaker/engine* trees) so they're version-controllable and survive reboots.
...
# (~/.mazemaker/engine/memory.db). The default below points at a benchmark sandbox
...
HOT_PATH_DB = (Path.home() / ".mazemaker" / "engine" / "memory.db").resolve()
SANDBOX_DB = Path.home() / ".mazemaker_benchmark" / "sandbox.db"
