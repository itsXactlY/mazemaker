# godbench — Mazemaker on LongMemEval-oracle (the hard variant)
...
This document is the full overview of the godbench iteration loop —
what we did, why each lever worked or didn't, and how to reproduce
the champion result.

`benchmarks/mazemaker_godbench.py` is the runner. It evaluates
...
3. **`benchmarks/mazemaker_godbench.py`** with the `_preference_query()`
...
`benchmarks/external/results/godbench_champion_<timestamp>.json`.
...
MAZEMAKER_SALIENCE_WEIGHT=0.5 \
python benchmarks/mazemaker_godbench.py --variant oracle --limit 500 \
...
```
benchmarks/
├── GODBENCH_GUIDE.md                ← this file
├── README.md                        ← internal benchmark suite overview
├── RESULTS.md                       ← pure-memory-bench results
├── mazemaker_godbench.py            ← godbench runner (oracle/S variants)
...
- `invariant_godbench_vs_v8_harness_mismatch.md` — godbench scope
