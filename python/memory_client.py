...
class Mazemaker:
    ...
    def _get_dream_engine(self):
        ...
        if self._dream_engine_singleton is None:
            ...
            # Pick the backend that matches self.store (the live memory
            # store). Was hard-coded to DreamEngine.sqlite — so a PG-
            # backed Mazemaker would run on-demand dream phases against
            # the live pgvector corpus but persist dream_sessions /
            # dream_insights / connection_history into an empty SQLite
            # file. Stage S synthesis then saw 0 cluster anchors and
            # the architect dashboard's dream metrics flatlined.
            # 2026-05-21 — matches the same fix on Memory._get_dream_engine
            # one layer up.
            backend_choice = (os.environ.get("MM_DB_BACKEND") or "").strip().lower()
            if backend_choice == "postgres":
                self._dream_engine_singleton = DreamEngine.postgres(neural_memory=self)
            else:
    ...
    def dream_config(self, action: str = "get", **kwargs) -> dict:
        ...
        When MM_DREAM_DISABLED=1 (external dream_worker.py owns the
        consolidation loop), `get` returns a {"external_daemon": True}
        sentinel rather than raising — callers can still inspect the
        cadence config from the running daemon's perspective via
        `dream_worker.py` flags. `set` returns an error in that mode
        because we can't reach into the external process.
        """
        import os
        external = os.getenv("MM_DREAM_DISABLED", "").lower() in ("1", "true", "yes")
        ...
        if external:
            if action == "set":
                return {
                    "ok": False,
                    "action": "set",
                    "error": "in-pod dream engine disabled (MM_DREAM_DISABLED=1)",
                    "hint": "tweak the dream_worker.py CLI flags (--max-memories, "
                            "--max-isolated, --cycle-interval) and restart that daemon",
                }
            return {
                "ok": True,
                "action": "get",
                ...
                "current": None,
                "note": "in-pod dream engine disabled; cadence is owned by "
                        "the external dream_worker.py process",
            }
        eng = self._get_dream_engine()
        known = (
            "idle_threshold",
    ...
    def connections_import_by_label(self, rows: list) -> dict:
        ...
        Peers can't ship raw memory IDs (every pod has its own
        AUTOINCREMENT id namespace), so /peer/connections/pull exports
        edges as {a_label, b_label, weight, edge_type, created_at}.
        This receiver resolves each label to the local memory id and
        upserts via the existing add_connection path. If either label
        doesn't resolve locally (memory hasn't synced over yet), the
        edge is skipped — next pull cycle, after the missing memory
        has come in via /peer/sync, the edge gets retried because the
        puller cursor stays one tick behind on edges.

        Returns: {imported, skipped_missing_endpoint, skipped_invalid}.
        """
        if self.store is None:
            return {"imported": 0, "skipped_missing_endpoint": 0,
                    "skipped_invalid": 0, "error": "no store"}
        if not isinstance(rows, list):
            return {"imported": 0, "skipped_missing_endpoint": 0,
                    "skipped_invalid": 0, "error": "rows must be a list"}

        imported = 0
        missing = 0
        invalid = 0
        # Cache label→id resolutions for this batch — peer edges often
        # cluster around a few hot memories, so the cache pays back.
        label_to_id: dict[str, int] = {}

        def _resolve(lbl: str) -> Optional[int]:
            if not lbl:
                return None
            if lbl in label_to_id:
                return label_to_id[lbl]
            hits = self.store.find_by_label(lbl) or []
            if not hits:
                label_to_id[lbl] = None  # negative cache for the batch
                return None
            # find_by_label can return multiple rows on label collisions
            # (rare but possible — the auto:turn:* labels are timestamped
            # so they're unique in practice). Use the lowest id, which is
            # the original (later rows are usually supersession revisions).
            chosen = min(hits, key=lambda h: int(h.get("id", 0)))
            label_to_id[lbl] = int(chosen["id"])
            return label_to_id[lbl]
        ...
        for r in rows:
            if not isinstance(r, dict):
                invalid += 1
                continue
            a_label = r.get("a_label") or ""
            b_label = r.get("b_label") or ""
            if not a_label or not b_label:
                invalid += 1
                continue
            a_id = _resolve(a_label)
            b_id = _resolve(b_label)
            ...
            try:
                ...
                invalid += 1
                continue
            edge_type = str(r.get("edge_type") or "similar")
            try:
                self.store.add_connection(a_id, b_id, weight, edge_type=edge_type)
                imported += 1
            except Exception:  # noqa: BLE001 — never block the batch on one bad row
                invalid += 1

        return {
            "imported": imported,
            "skipped_missing_endpoint": missing,
    ...
    def dream_control(self, action: str = "status") -> dict:
        ...
        When MM_DREAM_DISABLED=1 (external dream_worker.py owns the
        consolidation loop), `status` reports {"external_daemon": True}
        instead of raising. pause/resume return an error in that mode
        because we can't signal the external process from here — use
        `systemctl --user start/stop mazemaker-dream-worker.service`
        on the host instead.
        """
        ...
        if os.getenv("MM_DREAM_DISABLED", "").lower() in ("1", "true", "yes"):
            if action in ("pause", "resume"):
                return {
                    ...
                    "action": action,
                    "error": "in-pod dream engine disabled (MM_DREAM_DISABLED=1)",
                    "hint": "control the external daemon with "
                            "`systemctl --user {start,stop} mazemaker-dream-worker.service`",
                }
            return {
                "ok": True,
                ...
                "external_daemon": True,
                "running": "external",
                "note": "in-pod dream engine disabled; status owned by the "
                        "external dream_worker.py process",
            }
        eng = self._get_dream_engine()
        if action == "pause":
            eng.stop()
