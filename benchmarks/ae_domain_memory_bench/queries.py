...
_ELC = [
    ...
    _q("ELC-002", "electrical_contracting", "Find the service upgrade notes involving a 200 amp panel.", ["sparse","entity","temporal"], ground_truth_ids=[11368, 11828, 11910]),
    ...
    _q("ELC-007", "electrical_contracting", "What permit issues came up for service mast replacement?", ["sparse","temporal","entity"], ground_truth_ids=[14784, 14785]),
    _q("ELC-008", "electrical_contracting", "Which project involved EMT conduit and exposed garage wiring?", ["sparse","entity","graph"]),
    _q("ELC-009", "electrical_contracting", "Find memories about load calculation for a service upgrade.", ["sparse","dense","procedural"], ground_truth_ids=[5961]),
    ...
    _q("ELC-015", "electrical_contracting", "Find the memory about meter socket replacement.", ["sparse","entity"], ground_truth_ids=[11368, 11936]),
    ...
    _q("ELC-022", "electrical_contracting", "What did we learn about Naperville bond and permit-office filings?", ["entity","temporal","sparse"], ground_truth_ids=[13688, 14787]),
...
_MAT = [
    ...
    _q("MAT-011", "materials_sku", "Find notes about half inch conduit.", ["sparse","materials"], ground_truth_ids=[10780, 10822]),
    ...
    _q("MAT-014", "materials_sku", "What project needed weatherproof boxes?", ["sparse","entity","materials"], ground_truth_ids=[10948]),
    ...
    # MAT-018..020 unchanged below; new SKU labels for 14-AWG MC armored cable + THHN by-the-foot
    ...
    # MAT-022 unlabeled — no canonical disconnect SKU memory in HD catalog yet
    ...
    # MAT-027 stays unlabeled (no canonical low-voltage SKU in HD catalog yet)
...
_LOT = [
    ...
    # LOT-002 unchanged; below: new Braymore/Dartmouth permit-doc query
    ...
    _q("LOT-014", "lennar_lots", "Find notes about Lennar builder contact before Sarah.", ["entity","temporal"], ground_truth_ids=[264, 280]),
    _q("LOT-015", "lennar_lots", "Which lot had a missing GFCI?", ["entity","sparse"], ground_truth_ids=[277, 288]),
    _q("LOT-016", "lennar_lots", "Find work order memory for Lennar electrical trim.", ["entity","sparse"], ground_truth_ids=[9784, 9785, 9793]),
    ...
    # LOT-018 stays unlabeled (no canonical lot-18 record yet)
    _q("LOT-019", "lennar_lots", "What was the quote status for lot 51?", ["entity","temporal","financial"]),
    # LOT-019 unlabeled (no lot-51 specific memory)
    ...
    # LOT-021 stays unlabeled (no Lennar-specific subpanel memory)
    ...
    _q("LOT-025", "lennar_lots", "Find notes about Braymore Naperville lot panel permit.", ["entity","temporal"], ground_truth_ids=[13692, 14784]),
    _q("LOT-026", "lennar_lots", "Which lot had change order discussion?", ["entity","financial","temporal"]),
    _q("LOT-027", "lennar_lots", "Find all memories connected to Lennar superintendent.", ["entity","graph"], ground_truth_ids=[264, 268, 280, 282]),
    ...
    _q("LOT-033", "lennar_lots", "Find lot notes about Dartmouth Naperville Hernan permit drawings.", ["entity","temporal"], ground_truth_ids=[13693, 14785]),
    ...
    # LOT-035 unchanged; below: new permit-doc-anchored lot query
...
_FIN = [
    ...
    _q("FIN-011", "financial_calendar", "Find notes about permit fee reimbursement.", ["financial","sparse"], ground_truth_ids=[13687, 13688, 13692, 13693, 14786, 14787]),
    ...
    _q("FIN-037", "financial_calendar", "Find memories about insurance or certificate paperwork.", ["financial","sparse"], ground_truth_ids=[14787]),
...
_TMP = [
    ...
    # TMP-006 unlabeled (no Miguel/predecessor memory in canonical yet)
