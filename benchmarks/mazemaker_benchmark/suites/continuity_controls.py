...
Expected curve shape:
  ...
  * mazemaker may or may not beat raw cosine; if it doesn't, that's an
...
class ContinuityControlsBenchmark:
    def __init__(
        ...
        self.output_dir = output_dir or Path.home() / ".mazemaker_benchmark" / "results"
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for tier_i, count in enumerate(self.noise_tiers):
            ...
            tier = {
                ...
                "mazemaker": _eval(lambda q, k=5: nm.recall(q, k=k), target_queries, self.k),
            ...
            print(f"    {tier_id}:  nm={tier['mazemaker']['recall_at_k']}  "
                  f"raw={tier['raw_cosine']['recall_at_k']}  "
                  f"recency={tier['recency_only']['recall_at_k']}")

        # Curve analysis.
        nm_curve = [v["mazemaker"]["recall_at_k"] for v in results["tiers"].values()]
